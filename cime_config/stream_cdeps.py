...
# pylint: disable=undefined-variable #defined in standard_script_setup (CIME)
...
class StreamCDEPS(GenericXML):
        ...
        for stream_name in stream_names:
            ...
            for node in self.get_children(root=self.stream_nodes):
                ...
                if node_name == "stream_datavars":
                    ...
                    subcpl7names = self._check_for_cpl7_input(datafile)
                    ...
                    for child in self.get_children(root=node):
                        ...
                        if subcpl7names:
                            datavars = self._sub_atm_fields(datavars)
                    ...
                    for child in self.get_children(root=node):
                        ...
                        if (
                            ...
                            stream_datafiles = self._sub_paths(
                                stream_datafiles, year_first, year_last
                            )
                            stream_datafiles = stream_datafiles.strip()
                        # endif
                        datafile = stream_datafiles.partition("\n")[0]
                    ...
                    expect(False, f"Should I be here? {node_name}")
            ...
            stream_file_text = case.get_resolved_value(
                _stream_file_template.format(**stream_vars)
            )
            if "UNSET" in stream_file_text or (subcpl7names and "ndep" in stream_name):
                logger.warn(
                    f"WARNING: Stream {stream_name} contains no data files - skipping stream"
                )
            else:
    ...
    def _check_for_cpl7_input(self, datafile):
        # Try importing netcdf, it is an optional module for cime
        chkfilename = "." + os.path.basename(datafile) + "file_version"
        if os.path.exists(chkfilename):
            with open(chkfilename, "r") as fr:
                file_version = " ".join(fr.readlines())

            logger.debug(f"here file_version is {file_version}")
            return "cpl7" in file_version
        netcdf = self._import_netcdf()
        file_version = None
        if netcdf and os.path.exists(datafile):
            nc_fid = netcdf.Dataset(datafile, "r")
            nc_attrs = nc_fid.ncattrs()
            if "file_version" in nc_attrs:
                file_version = nc_fid.getncattr("file_version")
                logger.debug(f"file_version is {file_version}")
                with open(chkfilename, "w") as fw:
                    fw.write(file_version)
                if "cpl7" in file_version:
                    return True
        return False

    @staticmethod
    def _sub_atm_fields(datavars):
        """Substitute indicators with given values in a list of fields.
        We need to know if this cplhist file was created by mct or by cdeps.
        """
        cpl7prefix = "a2x1d_"
        cdepsprefix = "atmImp_"
        cpl7names = (
        ...
        newdatavars = datavars.replace(cdepsprefix, cpl7prefix, 1)
        for cdepsname, cpl7name in zip(cdepsnames, cpl7names):
            if cdepsname in datavars:
                newdatavars = newdatavars.replace(cdepsname, cpl7name, 1)
        return newdatavars
    ...
    def _import_netcdf():
        ...
        if not netCDF4:
            logger.info("No netCDF4 python module found.")
        return netCDF4
