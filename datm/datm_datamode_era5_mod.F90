module datm_datamode_era5_mod
  ...
  real(cdeps_real_kind), pointer :: Sa_wspd10m(:)        => null()
  real(cdeps_real_kind), pointer :: Sa_t2m(:)            => null()
  real(cdeps_real_kind), pointer :: Sa_tskn(:)           => null()
  ...
  real(cdeps_real_kind), pointer :: Faxa_swdn(:)         => null()
  real(cdeps_real_kind), pointer :: Faxa_swnet(:)        => null()
  real(cdeps_real_kind), pointer :: Faxa_lwdn(:)         => null()
  real(cdeps_real_kind), pointer :: Faxa_lwnet(:)        => null()
  real(cdeps_real_kind), pointer :: Faxa_sen(:)          => null()
  real(cdeps_real_kind), pointer :: Faxa_lat(:)          => null()
  real(cdeps_real_kind), pointer :: Faxa_taux(:)         => null()
  real(cdeps_real_kind), pointer :: Faxa_tauy(:)         => null()
!
!  real(cdeps_real_kind), pointer :: Faxa_ndep(:,:)       => null()
  ...
  real(cdeps_real_kind) :: t2max  ! units detector
  real(cdeps_real_kind) :: td2max ! units detector
...
!===============================================================================
  ...
  subroutine datm_datamode_era5_advance(exportstate, mainproc, logunit, mpicom, target_ymd, target_tod, model_calendar, rc)
    ...
    real(cdeps_real_kind) :: t2, pslv
    real(cdeps_real_kind) :: e, qsat
    type(ESMF_VM) :: vm
    ...
    do n = 1, lsize
       ...
       if (associated(Sa_t2m) .and. associated(Sa_pslv) .and. associated(Sa_q2m)) then
         t2 = Sa_t2m(n)
         pslv = Sa_pslv(n)
         if (td2max < 50.0_cdeps_real_kind) strm_tdew(n) = strm_tdew(n) + tkFrz
         e = datm_eSat(strm_tdew(n), t2)
         qsat = (0.622_cdeps_real_kind * e)/(pslv - 0.378_cdeps_real_kind * e)
         Sa_q2m(n) = qsat
       end if
    end do
    ...
    if (associated(Faxa_lwdn))  Faxa_lwdn(:)  = Faxa_lwdn(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_lwnet)) Faxa_lwnet(:) = Faxa_lwnet(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swvdr)) Faxa_swvdr(:) = Faxa_swvdr(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swndr)) Faxa_swndr(:) = Faxa_swndr(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swvdf)) Faxa_swvdf(:) = Faxa_swvdf(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swndf)) Faxa_swndf(:) = Faxa_swndf(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swdn))  Faxa_swdn(:)  = Faxa_swdn(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_swnet)) Faxa_swnet(:) = Faxa_swnet(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_sen))   Faxa_sen(:)   = Faxa_sen(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_lat))   Faxa_lat(:)   = Faxa_lat(:)/3600.0_cdeps_real_kind

    ! convert m to kg/m^2/s
    if (associated(Faxa_rain))  Faxa_rain(:)  = Faxa_rain(:)/3600.0_cdeps_real_kind*rhofw
    if (associated(Faxa_rainc)) Faxa_rainc(:) = Faxa_rainc(:)/3600.0_cdeps_real_kind*rhofw
    if (associated(Faxa_rainl)) Faxa_rainl(:) = Faxa_rainl(:)/3600.0_cdeps_real_kind*rhofw
    if (associated(Faxa_snowc)) Faxa_snowc(:) = Faxa_snowc(:)/3600.0_cdeps_real_kind*rhofw
    if (associated(Faxa_snowl)) Faxa_snowl(:) = Faxa_snowl(:)/3600.0_cdeps_real_kind*rhofw

    ! convert N/m^2 s to N/m^2
    if (associated(Faxa_taux))  Faxa_taux(:)  = Faxa_taux(:)/3600.0_cdeps_real_kind
    if (associated(Faxa_tauy))  Faxa_tauy(:)  = Faxa_tauy(:)/3600.0_cdeps_real_kind
